...
class HaDevice:
    ...
    def _get_entity_id_by_translation_key(
        ...
        """
        entity: Optional[RegistryEntry] = next(
            (e for e in self.entities if e.translation_key == entity_translation_key),
            None,
        )
        if entity is None:
            raise ValueError(
                f"Entity not found for translation_key '{entity_translation_key}'"
            )
        if entity.disabled:
            _LOGGER.error(
                f"Required entity {entity.entity_id} is disabled. Please enable it!"
            )
        return entity.entity_id

    def _get_entity_state(
        self, entity_id: str, parser_fn: Callable = None
    ) -> Optional[Any]:
        """
    ...
    def _get_entity_state_by_translation_key(
        self, entity_translation_key: str, parser_fn: Callable = None
    ) -> Optional[Any]:
        """
